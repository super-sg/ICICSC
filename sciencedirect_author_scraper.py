...
class ScienceDirectAuthorScraper:
    ...
    async def execute_topics(
        ...
        max_pages: int = 1,
        max_articles_per_topic: int = 4,
        output_path: str = "Conference_Tracks_Foreign_Authors.xlsx"
    ) -> List[Dict[str, Any]]:
        """
        Iterates through a queue of topics, scrapes articles, and collects foreign author records.
        Saves intermediate checkpoints after each topic.
        """
        ...
        for item in topic_queue:
            ...
            if all_results:
                ...
                logger.info(f"[Checkpoint Saved] Total records so far: {len(all_results)} -> {output_path}")
