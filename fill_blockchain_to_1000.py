...
def main():
    print("=================================================================")
    print("TOPPING UP TRACK 5 (BLOCKCHAIN) TO REACH EXACTLY 1,000 AUTHORS")
    print("=================================================================")
    
    excel_file = "Conference_Foreign_Authors_7000_Balanced.xlsx"
    xl = pd.ExcelFile(excel_file)
    
    track_pools: Dict[str, List[Dict[str, Any]]] = {}
    ...
    for sheet_name, t_label in sheet_map.items():
        ...
        for _, r in df_sheet.iterrows():
            ...
            if em not in seen_emails and is_legit_name(nm):
                rec = {
                    'Author Name': nm,
                ...
                rows.append(rec)
                seen_emails.add(em)
        track_pools[t_label] = rows
        print(f"Loaded {t_label}: {len(rows)} records")

    # Top up Track 5 (Blockchain)
    b_needed = 1000 - len(track_pools["Track 5: Blockchain"])
    print(f"\nBlockchain needed: {b_needed}")
    
    if b_needed > 0:
        b_queries = [
            'OPEN_ACCESS:Y AND (ABSTRACT:"blockchain" AND ABSTRACT:"security")',
            'OPEN_ACCESS:Y AND (ABSTRACT:"blockchain" AND ABSTRACT:"iot")',
            'OPEN_ACCESS:Y AND (TITLE:"consensus" OR TITLE:"proof of stake" OR TITLE:"proof of work")',
            'OPEN_ACCESS:Y AND (ABSTRACT:"smart contracts" AND ABSTRACT:"ethereum")'
        ]
        ...
        for q in b_queries:
            if len(candidate_pmcids) >= b_needed * 4:
                break
        ...
        print(f"Gathered {len(candidate_pmcids)} candidate papers for Blockchain. Parsing XML...")
    ...
    # Ensure every single track is capped at exactly 1000
    for t_label in track_pools:
        track_pools[t_label] = track_pools[t_label][:1000]

    print("\nFINAL EQUAL COUNTS ACROSS ALL 7 TRACKS:")
    ...
    print("\n=================================================================")
    print("PERFECT 7,000 EQUAL BALANCED DATASET COMPLETED (1,000 PER TRACK)!")
    print("=================================================================")
