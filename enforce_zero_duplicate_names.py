...
TRACK_QUERIES = [
    {
        ...
        "sheet": "Track 1",
        "queries": [
            'OPEN_ACCESS:Y AND (TITLE:"diffusion model" OR TITLE:"graph neural network")',
            'OPEN_ACCESS:Y AND (TITLE:"generative adversarial" OR TITLE:"explainable ai")',
            'OPEN_ACCESS:Y AND (TITLE:"few shot learning" OR TITLE:"meta learning")'
        ]
    },
    {
        ...
        "sheet": "Track 2",
        "queries": [
            'OPEN_ACCESS:Y AND (TITLE:"smart grid" OR TITLE:"vehicular network")',
            'OPEN_ACCESS:Y AND (TITLE:"narrowband iot" OR TITLE:"nb-iot")',
            'OPEN_ACCESS:Y AND (TITLE:"energy harvesting" AND TITLE:"sensor")'
        ]
    },
    {
        ...
        "sheet": "Track 3",
        "queries": [
            'OPEN_ACCESS:Y AND (TITLE:"surgical robot" OR TITLE:"swarm robotics")',
            'OPEN_ACCESS:Y AND (TITLE:"underwater vehicle" OR TITLE:"exoskeleton")',
            'OPEN_ACCESS:Y AND (TITLE:"mobile robot navigation" OR TITLE:"path planning")'
        ]
    },
    {
        ...
        "sheet": "Track 4",
        "queries": [
            'OPEN_ACCESS:Y AND (TITLE:"feature selection" OR TITLE:"sentiment analysis")',
            'OPEN_ACCESS:Y AND (TITLE:"anomaly detection" OR TITLE:"high dimensional data")',
            'OPEN_ACCESS:Y AND (TITLE:"stream mining" OR TITLE:"multivariate time series")'
        ]
    },
    {
        ...
        "sheet": "Track 5",
        "queries": [
            'OPEN_ACCESS:Y AND (ABSTRACT:"blockchain" AND ABSTRACT:"transaction")',
            'OPEN_ACCESS:Y AND (ABSTRACT:"blockchain" AND ABSTRACT:"security")',
            'OPEN_ACCESS:Y AND (ABSTRACT:"blockchain" AND ABSTRACT:"privacy")',
            'OPEN_ACCESS:Y AND (TITLE:"smart contracts" OR TITLE:"distributed ledger")',
            'OPEN_ACCESS:Y AND (TITLE:"hyperledger" OR TITLE:"cryptocurrency")'
        ]
    },
    {
        ...
        "sheet": "Track 6",
        "queries": [
            'OPEN_ACCESS:Y AND (TITLE:"ransomware" OR TITLE:"phishing detection")',
            'OPEN_ACCESS:Y AND (TITLE:"side channel attack" OR TITLE:"adversarial attack")',
            'OPEN_ACCESS:Y AND (TITLE:"denial of service" OR TITLE:"ddos" OR TITLE:"botnet")'
        ]
    },
    {
        ...
        "sheet": "Track 7",
        "queries": [
            'OPEN_ACCESS:Y AND (TITLE:"visual tracking" OR TITLE:"semantic segmentation")',
            'OPEN_ACCESS:Y AND (TITLE:"edge cloud" OR TITLE:"face recognition")',
            'OPEN_ACCESS:Y AND (TITLE:"serverless architecture" OR TITLE:"federated cloud")'
        ]
    }
]
...
def write_styled_workbook(track_pools: Dict[str, List[Dict[str, Any]]], filename: str):
    ...
    # Master Tab: exactly 7,000 authors
    all_master_rows = []
    track_order = [
...
def main():
    print("=================================================================")
    print("ENFORCING ZERO DUPLICATE NAMES ACROSS ALL 7,000 AUTHORS")
    print("=================================================================")
    ...
    print("Loading existing dataset and purging ANY duplicate names globally...")
    ...
    print("\nUnique Name Counts by Track (after global name & email deduplication):")
    total_unique_baseline = 0
    for t_label, rows in track_pools.items():
        needed = 1000 - len(rows)
        total_unique_baseline += len(rows)
        print(f"  - {t_label}: {len(rows)} / 1000 (needs {needed} fresh unique authors)")
    print(f"Total globally unique authors loaded: {total_unique_baseline}")

    # Top up each track with brand-new authors who have NEVER appeared before
    for cfg in TRACK_QUERIES:
        t_label = cfg['track']
        needed = 1000 - len(track_pools[t_label])
        if needed <= 0:
            print(f"\n✓ {t_label} already has 1,000 unique authors!")
            continue
            
        print(f"\n--- Harvesting fresh unique authors for {t_label} (Current: {len(track_pools[t_label])}, Needed: {needed}) ---")
        
        candidate_pmcids = []
        seen_pmcids = set()
        
        for q in cfg['queries']:
            if len(candidate_pmcids) >= needed * 4:
                break
        ...
        print(f"  Found {len(candidate_pmcids)} candidate papers. Fetching XMLs concurrently...")
            ...
            for future in as_completed(future_to_pmcid):
                ...
                try:
                    ...
                    for auth in res_authors:
                        ...
                        # STRICT GLOBAL DEDUPLICATION: zero duplicate names anywhere!
        ...
        print(f"  ✓ {t_label} now has: {len(track_pools[t_label])} / 1000")

    # Final trim to ensure each track has EXACTLY 1,000
    ...
    print("\n=================================================================")
    print("SUCCESS: 7,000 AUTHORS WITH ZERO DUPLICATE NAMES & EMAILS!")
    print("=================================================================")
